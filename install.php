...
if ($isInstalled && !isset($_GET['force'])) {
    echo '<!DOCTYPE html><html lang="ko"><head><meta charset="utf-8"><title>SpeedMIS (MSSQL)</title></head><body style="font-family:sans-serif;text-align:center;padding:80px">';
    echo '<h2>SpeedMIS v7 (MSSQL) 은 이미 설치되어 있습니다.</h2>';
...
<?php if ($step === 1): // ── DB 접속정보 ── ?>
  <span class="tag">MSSQL EDITION</span>
  ...
  <form method="post">
    ...
    <div class="row">
      ...
      <input type="text" name="db_host" value="<?= htmlspecialchars($_POST['db_host'] ?? '127.0.0.1') ?>" placeholder="127.0.0.1 또는 서버\인스턴스" required>
    </div>
    <div class="row2">
      <div>
        <label>DB 포트</label>
        <input type="number" name="db_port" value="<?= htmlspecialchars($_POST['db_port'] ?? '1433') ?>" placeholder="1433">
      </div>
      <div>
        <label>DB 이름</label>
    ...
    <div class="row2">
      <div>
        <label>DB 사용자</label>
        <input type="text" name="db_user" value="<?= htmlspecialchars($_POST['db_user'] ?? 'sa') ?>" required>
      </div>
      <div>
        <label>DB 비밀번호</label>
...
<?php elseif ($step === 3): // ── 완료 ── ?>
  ...
  <p class="sub" style="text-align:center">SpeedMIS v7 (MSSQL) 이 성공적으로 설치되었습니다.</p>
  <?php if (!empty($log)): ?><div class="log"><?php foreach ($log as $l): ?><?= htmlspecialchars($l) ?><br><?php endforeach; ?></div><?php endif; ?>
  <div class="ok">
