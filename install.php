<?php
/**
 * SpeedMIS v7 (MSSQL 배포판) — Install Wizard
 *
 * 워드프레스식 최초 구동 설치:
 *   1) MSSQL 접속정보 입력 → 연결 테스트 → DB 생성(없으면)
 ...
 *
 * ⚠ 이 레포(speedmis_v7_mssql)는 MariaDB 배포판(clude_speedmis_v7 / speedmis_v7)과 별개입니다.
 *    설치는 Microsoft SQL Server 전용입니다.
 */
...
const DB_BUNDLE_URL_DEFAULT = 'https://raw.githubusercontent.com/speedmis/speedmis_v7_mssql/main/db/speedmis_db.sql.gz';
...
// ── force 모드: 기존 객체 전부 DROP (FK 우선) ────────────────────────────
function drop_all_objects_mssql(PDO $pdo, array &$log): void
{
    $tCount = 0; $vCount = 0; $fCount = 0;
    try {
        // 1) FK 먼저 제거 (DROP TABLE 순서 의존성 제거)
        $fks = $pdo->query("SELECT name, OBJECT_NAME(parent_object_id) AS tbl FROM sys.foreign_keys")->fetchAll(PDO::FETCH_ASSOC);
        foreach ($fks as $fk) {
            try { $pdo->exec("ALTER TABLE [{$fk['tbl']}] DROP CONSTRAINT [{$fk['name']}]"); $fCount++; } catch (\Throwable) {}
        }
        // 2) View
        $views = $pdo->query("SELECT TABLE_NAME FROM INFORMATION_SCHEMA.VIEWS WHERE TABLE_SCHEMA='dbo'")->fetchAll(PDO::FETCH_COLUMN);
        foreach ($views as $v) {
            try { $pdo->exec("DROP VIEW [{$v}]"); $vCount++; } catch (\Throwable) {}
        }
        // 3) Table
        $tables = $pdo->query("SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_TYPE='BASE TABLE' AND TABLE_SCHEMA='dbo'")->fetchAll(PDO::FETCH_COLUMN);
        foreach ($tables as $t) {
            try { $pdo->exec("DROP TABLE [{$t}]"); $tCount++; } catch (\Throwable) {}
        }
    } catch (\Throwable $e) {
        $log[] = "객체 정리 단계 경고: " . $e->getMessage();
    }
    $log[] = "기존 객체 정리: 테이블 {$tCount}개, 뷰 {$vCount}개, FK {$fCount}개 DROP (force 재적재)";
}
...
if ($step === 2 && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $dbPort    = trim($_POST['db_port'] ?? '1433');
    ...
    if (!extension_loaded('pdo_sqlsrv')) {
        $errors[] = 'PHP pdo_sqlsrv 확장이 설치되어 있지 않습니다. (Microsoft Drivers for PHP for SQL Server 필요)';
    }

    // 1) 서버 접속 + DB 생성 (공유 호스팅은 CREATE DATABASE 권한이 없을 수 있어 graceful fallback)
    if (empty($errors)) {
        try {
            $dsnServer = "sqlsrv:Server={$dbHost},{$dbPort};TrustServerCertificate=true;Encrypt=optional;LoginTimeout=5";
            $pdo = new PDO($dsnServer, $dbUser, $dbPass, [PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION]);

            $dbId = $pdo->query("SELECT DB_ID(" . $pdo->quote($dbName) . ")")->fetchColumn();
            if ($dbId === null || $dbId === false || $dbId === '') {
                try {
    ...
    // 2) DB 선택 후 기설치 여부 확인 (force 모드는 검사 스킵하고 기존 객체 DROP)
    if (empty($errors)) {
        try {
            $dsnDb = "sqlsrv:Server={$dbHost},{$dbPort};Database={$dbName};TrustServerCertificate=true;Encrypt=optional;LoginTimeout=5";
            $pdo = new PDO($dsnDb, $dbUser, $dbPass, [PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION]);
            if (!$isForce) {
                $has = $pdo->query("SELECT OBJECT_ID('dbo.mis_menus','U')")->fetchColumn();
                if ($has) {
                    $errors[] = "'{$dbName}' 에 이미 mis_menus 테이블이 존재합니다. 빈 DB를 쓰거나 DB 이름을 바꾸세요.";
                }
            } else {
                // force 모드: 기존 객체 전부 DROP (MSSQL 번들은 IF OBJECT_ID IS NULL 패턴이라 명시적 DROP 필요)
                drop_all_objects_mssql($pdo, $log);
            }
        } catch (PDOException $e) {
    ...
    if (empty($errors) && !$isForce) {
        ...
        $env = <<<ENV
# SpeedMIS v7 (MSSQL 배포판) — install.php 가 자동 생성
DB_DRIVER=sqlsrv
DB_HOST={$dbHost}
DB_PORT={$dbPort}
DB_NAME={$dbName}
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>SpeedMIS v7 (MSSQL) Install</title>
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .tag { display:inline-block; font-size:11px; font-weight:700; color:#fff; background:#0a7d36; border-radius:4px; padding:2px 8px; margin-bottom:14px; letter-spacing:.5px; }
...
<?php if ($step === 1): // ── DB 접속정보 ── ?>
  <span class="tag"<?= $isForce ? ' style="background:#dc2626"' : '' ?>><?= $isForce ? 'FORCE — DB 재적재' : 'MSSQL EDITION' ?></span>
  <h1><?= $isForce ? 'DB 재적재 (force)' : 'SpeedMIS v7 설치' ?></h1>
  <?php if ($isForce): ?>
    <p class="sub" style="color:#b91c1c;font-weight:500"><strong>⚠ 위험:</strong> 기존 DB 의 모든 객체(테이블·뷰·FK) 가 DROP 되고 초기 상태(speedmis_db 번들)로 되돌아갑니다. <code>.env</code> 는 보존됩니다.</p>
  <?php else: ?>
    <p class="sub">Microsoft SQL Server 접속 정보를 입력하면, 초기 데이터를 자동으로 받아 설치합니다.</p>
  <?php endif; ?>
  ...
  <?php
    ...
    $f_dbPort = $_POST['db_port']    ?? ($isForce ? ($envData['DB_PORT']    ?? '1433')      : '1433');
    $f_dbName = $_POST['db_name']    ?? ($isForce ? ($envData['DB_NAME']    ?? 'speedmis_db') : 'speedmis_db');
    $f_dbUser = $_POST['db_user']    ?? ($isForce ? ($envData['DB_USER']    ?? 'sa')        : 'sa');
  ...
  <form method="post" <?= $isForce ? 'action="?force=1"' : '' ?>>
    ...
    <div class="row">
      <label>DB 호스트 (SQL Server)</label>
      <input type="text" name="db_host" value="<?= htmlspecialchars($f_dbHost) ?>" placeholder="127.0.0.1 또는 서버\인스턴스" required<?= $isForce ? ' readonly style="background:#f8f9fb;cursor:not-allowed"' : '' ?>>
    </div>
    <div class="row2">
      <div>
        <label>DB 포트</label>
        <input type="number" name="db_port" value="<?= htmlspecialchars($f_dbPort) ?>" placeholder="1433"<?= $isForce ? ' readonly style="background:#f8f9fb;cursor:not-allowed"' : '' ?>>
      </div>
      <div>
        <label>DB 이름</label>
    ...
    <div class="hint" style="margin-bottom:20px;color:#b91c1c">계속하면 dbo 스키마의 모든 테이블·뷰·FK 가 DROP 되고 초기 speedmis_db 번들로 재적재됩니다.</div>
    <?php endif; ?>
...
<?php elseif ($step === 3): // ── 완료 ── ?>
  ...
  <p class="sub" style="text-align:center">
    <?php if ($isForce): ?>
      SpeedMIS v7 (MSSQL) DB 가 초기 상태로 재적재되었습니다. <strong>.env 는 그대로 보존</strong>되었습니다.
    <?php else: ?>
      SpeedMIS v7 (MSSQL) 이 성공적으로 설치되었습니다.
    <?php endif; ?>
  </p>
  <?php if (!empty($log)): ?><div class="log"><?php foreach ($log as $l): ?><?= htmlspecialchars($l) ?><br><?php endforeach; ?></div><?php endif; ?>
  <div class="ok">
