# SpeedMIS v7 — MSSQL 배포판 (speedmis_v7_mssql)

> ⚠️ **이 레포는 `clude_speedmis_v7` / `speedmis_v7` (MariaDB 운영본)과 완전히 별개입니다.**
> 코드를 섞거나 혼동하지 마세요. 이 레포는 **Microsoft SQL Server 전용 다운로드 배포판**입니다.

워드프레스처럼 **다운로드 → 최초 구동 시 설치 마법사**로 DB 정보를 입력하면, 초기 데이터를 자동으로 받아 설치하는 독립 패키지입니다.

## 빠른 시작

1. 이 레포를 웹루트에 클론/압축해제 (PHP 8.3 + `pdo_sqlsrv` 확장 필요)
2. 브라우저에서 `https://<내도메인>/install.php` 접속
3. SQL Server 접속정보(호스트/포트/이름/계정) 입력 → **연결 & 자동 설치**
   - DB 가 없으면 자동 생성
...
- **MSSQL 전용** — `.env` 의 `DB_DRIVER=sqlsrv` 고정, `MssqlCompatPDO` 로 MariaDB 문법 자동 호환
...
| `core/`, `programs/`, `src/` | v7 애플리케이션 코드 (MariaDB 배포판과 동일 베이스) |

## 스택

PHP 8.3 + Slim 4 + React 18 + Vite / **Microsoft SQL Server**
