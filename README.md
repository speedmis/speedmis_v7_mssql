...
> ⚠️ **이 레포는 `speedmis_v7_mariadb` / `speedmis_v7_postgresql` 와 별개입니다.**
> 이 레포는 **Microsoft SQL Server 전용 다운로드 배포판**입니다.
...
```bash
git clone https://github.com/speedmis/speedmis_v7_mssql.git
...
- **MariaDB / MySQL 전용** — `.env` 의 `DB_DRIVER=sqlsrv` 고정
...
| **Microsoft SQL Server** | https://github.com/speedmis/speedmis_v7_mssql (이 레포) |
| PostgreSQL | https://github.com/speedmis/speedmis_v7_postgresql |

## 스택

PHP 8.3 + Slim 4 + React 18 + Vite / **Microsoft SQL Server 2019+**
